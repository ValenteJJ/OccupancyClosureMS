...
estDepDates = structure(list(bird_id = c("GT5", "WAM2", "WAM15", "NQP1", "WAM14", 
                                         ...
                                         "CNPS27", "CNPS39", "CNPS41", "NNP1", "WAM13", "DAMB"), departure_dt = structure(c(NA, 
                                                                                                                            NA, 15869, NA, 15864, NA, NA, NA, 15861, NA, NA, NA, NA, NA, 
                                                                                                                            15865, 15867, 15867, 15867, 15859, 15900, NA, 16239, NA, 16221, 
...
#If we identified one using the eyeball test, we will use a timestamp around that
#point. If we did not identify one with the eyeball test, we will use the midpoint
...
#Record the results
try({results1 = output$aic.table %>% mutate(bird_id = birdOrder$bird_id[bird])}, silent=T)
try({results2 = data.frame('bird_id' = birdOrder$bird_id[bird],
                           'lrt' = output$lrt,
                           'df' = output$df,
                           'pValue' = output$p.value)}, silent=T)
try({results3 = test$p.CI %>% 
