...
library(groundhog)
pkgs = c('RCurl', 'colorspace', 'cli', 'ggplot2', 'tidyverse', 'marcher')
...
capLocs = read.csv(text = getURL("https://raw.githubusercontent.com/vitekology/OccupancyClosureMS/main/capLocs.csv"))



#Estimated dates of range shifts based on eyeball test
estDepDates = read.csv(text = getURL("https://raw.githubusercontent.com/vitekology/OccupancyClosureMS/main/estDepDates.csv"))



#Read in telemetry locations
wothLocs = read.csv(text = getURL("https://raw.githubusercontent.com/vitekology/OccupancyClosureMS/main/wothLocations.csv")) %>% 
  mutate(dt = dmy_hm(dt)) %>%
